...
const CACHE_TTL = 10000; // 10 segundos de cache

// Headers optimizados con rotación de User-Agent
const getUserAgent = () => {
    const agents = [
    ...
    for (let i = 0; i < retries; i++) {
        try {
            ...
            if (response.status === 429) { // Rate limit
                console.log(`⏰ Rate limit detectado, esperando 1 segundo...`);
...
async function getUserStats(userId) {
    ...
    try {
        ...
        // Hacer solicitudes secuenciales para evitar rate limiting masivo
...
app.get('/', (req, res) => {
    res.json({ 
        status: 'online',
        message: 'Roblox Proxy Server - Fixed Rate Limiting',
        timestamp: new Date().toISOString()
    });
});
...
app.listen(port, () => {
    ...
    console.log(`🛡️  Protegido contra rate limiting`);
});
